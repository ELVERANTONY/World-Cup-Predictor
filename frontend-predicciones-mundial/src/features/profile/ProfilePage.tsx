    ...
    <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  {favoriteTeam && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2 justify-center sm:justify-start">Favorite Team: {favoriteTeam.flagUrl ? <img src={favoriteTeam.flagUrl.replace('w80/w80/', 'w80/')} alt="flag" className="w-5 h-3 object-cover rounded-sm" /> : ''} {favoriteTeam.name}</p>}
                </div>
                <Button variant="outline" size="sm" onClick={() => setShowEdit(true)} iconLeft={<Edit3 className="w-4 h-4" />}>Edit</Button>
              </div>
      ...
      {stats && (
        <motion.div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3" variants={staggerContainer} initial="hidden" animate="visible">
          {[
            { icon: Target, value: stats.predictionsCount, label: 'Predictions', color: 'text-blue-500 bg-blue-50 dark:bg-blue-500/10' },
            { icon: TrendingUp, value: `${stats.accuracyRate}%`, label: 'Accuracy', color: 'text-green-500 bg-green-50 dark:bg-green-500/10' },
            { icon: Trophy, value: stats.totalPoints.toLocaleString(), label: 'Points', color: 'text-amber-500 bg-amber-50 dark:bg-amber-500/10' },
            { icon: Medal, value: `#${stats.rank}`, label: 'Rank', color: 'text-purple-500 bg-purple-50 dark:bg-purple-500/10' },
            { icon: Flame, value: `${stats.currentStreak}`, label: 'Streak', color: 'text-red-500 bg-red-50 dark:bg-red-500/10' },
          ].map(stat => (
